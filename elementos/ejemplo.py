...
def cargar_datos(nombre_tabla,datos:list):
    with conexion() as conn:
        cursor= conn.cursor()
        lis = ['%s' for _ in range(len(datos))]
        cantidad = ','.join(lis)    
        query = f"INSERT INTO {nombre_tabla} VALUES ({cantidad})"
        cursor.execute(query,datos)
        conn.commit()

for i in range(len(lista)):
    valores = [i+1,'Perro',lista[i]]
    cargar_datos('especie',valores)
